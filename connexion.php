    ...
    // ETAPE de vérification des données
        if( empty($_POST['username']) || empty($_POST['password']) ){
            ...
            // ETAPE  de connexion
                ...
                // Récupération d'un membre via son pseudo uniquement
                ...
                if ( $requete->rowCount() == 1 ) {
                    ...
                    // Je vais donc ici aller chercher mon utilisateur à l'intèrieur de l'objet et je le stock dans la variable $user
                    $user = $requete->fetch();
                    // debug($user);    

                    // Maintenant que j'ai un utilisateur je peux essayer de vérifier son mot de passe.
                    // Le mdp en BDD est haché donc pour vérifier et comparer le mdp reçu dans le formulaire je dois utiliser la fonction password_verify()
                    // Cette fonction attend en premier paramètre le mpd 'normal' et en deuxième paramètre le mdp haché. La fonction renverra TRUE ou FALSE en fonction du résultat.
                    if( password_verify($_POST['password'], $user['password']) ){
                        ...
                        // $_SESSION['successMessage'] = "Bonjour". $_SESSION['membre']['username']. ", bienvenue sur votre compte";
                        
                        header('location:profil.php');
                        exit;

                    } else {
                        $errorMessage = "Mot de passe ou Indenfiant incorrects";
                    }
