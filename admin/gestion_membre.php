...
require_once "../inc/init.php";
            ...
            if( !empty($_POST) ){
                ...
                // Vérification des données 
                    ...
                    if(isset($_POST['status']) && !in_array($_POST['status'], ['user', 'admin'])){
                        $errorMessage .= "Le statut sélectionné n'est pas reconnu <br>";
                    }
    ...
    // ETAPE de suppression d'un membre
        // Je vérifie si l'indice 'action' dans la superglobale $_GET existe. S'il existe et que sa valeur est égale à 'supprimer' alors je rentre dans la condition.  
        if( isset($_GET['action']) && $_GET['action'] == "supprimer" ){
            ...
            if( $_SESSION['membre']['id'] != $_GET['id_membre'] ){
                ...
                $errorMessage = "Attention vous ne pouvez pas supprimer votre propre compte !";
            }
    ...
    // ETAPE de récupération des membres
        ...
        $membres = $requete->fetchAll(); // je vais chercher à l'intèrieur de l'objet PDOStatement les membres.
            ...
            <?php foreach( $membres as $membre ){ ?>
                <tr>
                    ...
                    <td>
                        ...
                        <a class="btn btn-sm btn-danger" href="gestion_membre.php?action=supprimer&id_membre=<?= $membre['id'] ?>" onclick="return confirm('Êtes-vous sur de vouloir supprimer ce membre ?')" >Supprimer</a>
                    </td>
                </tr>
            <?php } ?>
        </tbody>
...
// https://www.domaine.fr?action=supprimer&id=2
